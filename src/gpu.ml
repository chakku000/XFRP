...
let generate_gnode_update_kernel (name : string) (expr : Syntax.gexpr) (ast : Syntax.ast) (program: Module.program) : string = 
  (* Printf.printf "-------- %s -------------\n" name; *)
  let modification = "__global__" in
  let kernel_name = Printf.sprintf "%s_kernel" name in
  let id_to_type_table = (* Nodeから型への辞書 TODO これを毎回構築してるの効率が悪い *)
  ...
  let rec collect_node_atlast_deps e = 
    ...
    | GSelf -> StringSet.empty
    | GConst(_) -> StringSet.empty
    | Gid(_) ->  StringSet.empty
    | GAnnot(id,_) -> 
        if (List.exists (String.equal id) program.node) then (StringSet.singleton id) (* めんどいから適当に_at_lastとか付けてみた *)
        else StringSet.empty
    | Gbin(_,e1,e2) -> 
        StringSet.union (collect_node_atlast_deps e1) (collect_node_atlast_deps e2)
    | GApp(_,el) -> 
        List.fold_left (fun acc elm -> StringSet.union acc (collect_node_atlast_deps elm)) StringSet.empty el
    | Gif (ce,e1,e2) -> 
        StringSet.union (collect_node_atlast_deps ce) (StringSet.union (collect_node_atlast_deps e1) (collect_node_atlast_deps e2))
  in
  let node_set = collect_node_deps expr in
  let node_atlast_set = collect_node_atlast_deps expr in
  Hashtbl.iter (fun k v -> Printf.printf "%s - %s\n" k (Type.of_string v)) id_to_type_table;
  let normal_args = 
    StringSet.elements node_set |> List.map (fun nd -> Printf.sprintf "%s %s" (Hashtbl.find id_to_type_table nd |> Type.of_string) nd) |> String.concat ", " in
  let last_args = 
    StringSet.elements node_atlast_set |> List.map (fun nd -> Printf.sprintf "%s %s_ATLAST" (Hashtbl.find id_to_type_table nd |> Type.of_string) nd) |> String.concat "," in
  (* Printf.printf "normal_args : %s\n" normal_args; *)
  (* Printf.printf "last_args : %s\n"  last_args; *)
  let args = "(" ^ (String.concat "," (List.filter (fun l ->  String.length l > 0) [normal_args; last_args])) ^ ")" in
