...
let compile in_c : string = 
  ...
  try
    ...
    "Input Node = [" ^ (String.concat "," (List.map (fun (i,t) -> i) ast.in_nodes)) ^ "]\n" ^
    "Output Node = [" ^ (String.concat "," (List.map (fun (i,t) -> i) ast.out_nodes)) ^ "]\n" ^ 
...
let main () =
  ...
  try
    let input : in_channel = open_in (match !input_file with (* input::in_channel *)
                        | Some s -> s
                        | None -> raise (CommandError "Input file is not specified.")) in
    let c_code = compile input in
    print_endline c_code
  with
  | CommandError msg -> Printf.eprintf "Command Error: %s" msg
