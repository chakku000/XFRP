...
void red_update(){
	red_kernel<<<dim3(2400),dim3(512)>>>(red[turn], image[turn]);
}
...
void green_update(){
	green_kernel<<<dim3(2400),dim3(512)>>>(green[turn], image[turn]);
}
...
void blue_update(){
	blue_kernel<<<dim3(2400),dim3(512)>>>(blue[turn], image[turn]);
}
...
__global__ void grayscale_kernel(float* grayscale, float* grayscale_rg){
	int self = blockIdx.x * blockDim.x + threadIdx.x;
	if(self < 11228800){
		grayscale[self] = pow(grayscale_rg[self],2.2);
	}
}
...
void update_0_0(){
	for(int i=0;i<11228800;i++) grayImage_update(i);
}
void update_0_1(){
	grayscale_update();
}
...
void update_0_5(){
	for(int i=0;i<1228800;i++) image_update(i);
...
void setup(){
	...
	/* GPU grayscale_rg */
	for(int i=0;i<2;i++) cudaMalloc((void**)&g_grayscale_rg[i],1128800*sizeof(float));
	
	/* GPU grayscale */
	for(int i=0;i<2;i++) cudaMalloc((void**)&g_grayscale[i],11228800*sizeof(float));
}
